...
ggplot(data.frame(x=c(0.28,0.32)), aes(x))+
  ...
  stat_function(xlim = c(0.28, 0.3), aes(x),fun = dbeta, args = list(shape1 = alpha_single, shape2 = beta_single), geom = "area", fill = "blue", inherit.aes = F)
...
```{r}
career_eb %>% 
  ggplot(aes(eb_estimate, PEP, col= AB))+
  geom_point()
```
